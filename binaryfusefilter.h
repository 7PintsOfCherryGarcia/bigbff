...
static inline uint32_t binary_fuse8_hash(int index,
                                         uint64_t hash,
                                         const binary_fuse8_t *filter)
{
...
static inline uint64_t binary_fuse_calculate_segment_length(uint32_t arity,
                                                            uint64_t size)
{
    // These parameters are very sensitive. Replacing 'floor' by 'round' can
    // substantially affect the construction time.
    if (arity == 3)
        return ((uint64_t)1) << (int)(floor(log((double)(size)) / log(3.33) + 2.25));
    else if (arity == 4)
        return ((uint64_t)1) << (int)(floor(log((double)(size)) / log(2.91) - 0.5));
    else
        return 65536;
}
...
{
    uint32_t arity = 3;
    filter->SegmentLength = size == 0 ? 4 : binary_fuse_calculate_segment_length(arity,
                                                                                 size);
...
static inline bool binary_fuse8_populate(uint64_t *keys,
                                         uint32_t size,
                                         binary_fuse8_t *filter) {
...
static inline uint64_t binary_fuse16_hash(int index,
                                          uint64_t hash,
                                          const binary_fuse16_t *filter)
{
    uint64_t h = binary_fuse_mulhi(hash, filter->SegmentCountLength);
...
// before. For best performance, the caller should ensure that there are not too
// many duplicated keys.
static inline bool binary_fuse16_populate(uint64_t *keys,
                                          uint64_t size,
                                          binary_fuse16_t *filter) {
    uint64_t rng_counter = 0x726b2b9d438b9d4d;
    filter->Seed = binary_fuse_rng_splitmix64(&rng_counter);
    ...
    uint32_t blockBits = 1;
    while ( (1U << blockBits) < filter->SegmentCount)
        blockBits += 1;
    uint32_t block = 1U << blockBits;
    uint32_t *startPos = (uint32_t *)malloc( block * sizeof(uint32_t));
    uint32_t h012[5];

    if ( !alone  || !t2count      || !reverseH ||
         !t2hash || !reverseOrder || !startPos) {
        free(alone);
        free(t2count);
        free(reverseH);
        free(t2hash);
...
static inline uint64_t binary_fuse32_fingerprint(uint64_t hash)
{
    return hash ^ (hash >> 32);
}
...
static inline uint64_t binary_fuse32_hash(int index,
                                          uint64_t hash,
                                          const binary_fuse32_t *filter)
{
    uint64_t h = binary_fuse_mulhi(hash, filter->SegmentCountLength);
                                          ...
                                          binary_fuse32_t *filter)
{
    ...
    filter->ArrayLength =
        (filter->SegmentCount + arity - 1) * filter->SegmentLength;
    filter->SegmentCountLength = filter->SegmentCount * filter->SegmentLength;
    filter->Fingerprints = (uint32_t*)malloc(filter->ArrayLength * sizeof(uint32_t));
    memset(filter->Fingerprints, 0, filter->ArrayLength * sizeof(uint32_t));
    return filter->Fingerprints != NULL;
}
...
// before. For best performance, the caller should ensure that there are not too
// many duplicated keys.
static inline bool binary_fuse32_populate(uint64_t *keys,
                                          uint32_t size,
                                          binary_fuse32_t *filter) {
    uint64_t rng_counter = 0x726b2b9d438b9d4d;
    filter->Seed = binary_fuse_rng_splitmix64(&rng_counter);
